...
/////// Diffuse:
                ...
                float4 node_7618 = _Time;
                float2 node_2531 = (i.uv0+node_7618.g*float2(0.5,0));
                float4 _Clouds2_var = tex2D(_Clouds2,TRANSFORM_TEX(node_2531, _Clouds2));
                float3 diffuseColor = (i.vertexColor.rgb*_Clouds2_var.rgb);
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
////// Emissive:
                float2 node_122 = (i.uv0+node_7618.g*float2(0.5,-0.25));
                float4 _Clouds_var = tex2D(_Clouds,TRANSFORM_TEX(node_122, _Clouds));
                float3 emissive = (i.vertexColor.rgb*pow(_Clouds_var.rgb,0.75));
/// Final Color:
                float3 finalColor = diffuse + emissive;
                float4 _CloudsAlpha_var = tex2D(_CloudsAlpha,TRANSFORM_TEX(node_2531, _CloudsAlpha));
                fixed4 finalRGBA = fixed4(finalColor,((_Clouds2_var.rgb+0.5)*(1.0 - _CloudsAlpha_var.r)).r);
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_8669 = _Time;
                float2 node_2531 = (i.uv0+node_8669.g*float2(0.5,0));
                float4 _Clouds2_var = tex2D(_Clouds2,TRANSFORM_TEX(node_2531, _Clouds2));
                float3 diffuseColor = (i.vertexColor.rgb*_Clouds2_var.rgb);
...
/// Final Color:
                ...
                float4 _CloudsAlpha_var = tex2D(_CloudsAlpha,TRANSFORM_TEX(node_2531, _CloudsAlpha));
                fixed4 finalRGBA = fixed4(finalColor * ((_Clouds2_var.rgb+0.5)*(1.0 - _CloudsAlpha_var.r)).r,0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
