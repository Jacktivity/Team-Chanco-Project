...
////// Specular:
                ...
                float node_2732 = 3.0;
                float3 node_5680 = (_RTex_var.rgb/node_2732);
                float2 node_7977 = float2(i.posWorld.r,i.posWorld.b);
                float4 _GTex_var = tex2D(_GTex,TRANSFORM_TEX(node_7977, _GTex));
                float3 node_6008 = (_GTex_var.rgb/node_2732);
                float2 node_1853 = float2(i.posWorld.r,i.posWorld.b);
                float4 _BTex_var = tex2D(_BTex,TRANSFORM_TEX(node_1853, _BTex));
                float3 node_5473 = (_BTex_var.rgb/node_2732);
                float3 diffuseColor = (node_2700.r*_RTex_var.rgb + node_2700.g*(node_5680+node_6008+node_5473) + node_2700.b*_BTex_var.rgb); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _BTex_var = tex2D(_BTex,TRANSFORM_TEX(node_1853, _BTex));
                float3 node_5473 = (_BTex_var.rgb/node_2732);
                float3 diffColor = (node_2700.r*_RTex_var.rgb + node_2700.g*(node_5680+node_6008+node_5473) + node_2700.b*_BTex_var.rgb);
                float specularMonochrome;
                float3 specColor;
